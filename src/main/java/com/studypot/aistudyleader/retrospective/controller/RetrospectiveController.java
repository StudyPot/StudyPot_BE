...
import com.studypot.aistudyleader.retrospective.service.GetRetrospectiveOverviewQuery;
import com.studypot.aistudyleader.retrospective.service.ListMyRetrospectivesQuery;
import com.studypot.aistudyleader.retrospective.service.SubmitRetrospectiveCommand;
...
class RetrospectiveController {
	...
	RetrospectiveResponse submitRetrospective(
		Authentication authentication,
		@Parameter(description = "회고를 제출할 커리큘럼 주차 UUID입니다.", required = true)
		@PathVariable UUID weekId,
		@RequestBody(required = false) SubmitRetrospectiveRequest request
	) {
		Retrospective retrospective = service().submitMyRetrospective(new SubmitRetrospectiveCommand(
	...
	@Operation(
		summary = "내 그룹 회고 주차별 개요",
		description = "주차별로 회고 질문, 잠금 여부(필수 TODO 완료), 내 작성 여부를 한 번에 조회합니다."
	)
	@ApiResponses({
		@ApiResponse(responseCode = "200", description = "주차별 회고 개요 반환"),
		@ApiResponse(responseCode = "401", description = "인증된 사용자 정보를 확인할 수 없음"),
		@ApiResponse(responseCode = "403", description = "대상 그룹의 활성 멤버가 아니어서 조회할 수 없음"),
		@ApiResponse(responseCode = "503", description = "회고 서비스가 아직 구성되지 않음")
	})
	@GetMapping(ApiPaths.V1 + "/groups/{groupId}/retrospectives/overview")
	List<RetrospectiveWeekOverview> getRetrospectiveOverview(
		Authentication authentication,
		@Parameter(description = "회고 개요를 조회할 스터디 그룹 UUID입니다.", required = true)
		@PathVariable UUID groupId
	) {
		return service().getRetrospectiveOverview(new GetRetrospectiveOverviewQuery(authenticatedUserId(authentication), groupId));
	}
	...
	private record RetrospectiveResponse(
		...
		@Schema(description = "멤버가 제출한 회고 설문 답변(input_summary.answers). 미제출이면 null.")
		Object answers
	) {
		...
		private static RetrospectiveResponse from(Retrospective retrospective) {
			return new RetrospectiveResponse(
				...
				retrospective.inputSummary() == null ? null : retrospective.inputSummary().get("answers")
			);
		}
	}

	@Schema(description = "회고 설문 답변 제출 요청입니다.")
	private record SubmitRetrospectiveRequest(
		@Schema(description = "질문별 답변 목록. 각 항목은 {questionId, score(리커트 1~5), text(자유서술)}.")
