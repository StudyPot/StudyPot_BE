...
- `create-pr.sh`는 PR 생성 후 GitHub Copilot review를 요청하고, `finish-pr.sh`는 Copilot review activity와 미해결 thread 0개를 확인한 뒤에만 Mattermost manual merge 알림을 보냅니다.
...
- `finish-pr.sh`는 최신 PR head에 대한 `GitHub Actions Review Gate: PASS` marker와 Copilot review activity, 미해결 Copilot review thread 0개가 확인되지 않으면 manual merge 알림을 보내지 않습니다.
