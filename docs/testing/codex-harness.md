...
- `run-coderabbit-review.sh`는 PR 생성 후 PR target branch를 `--base`로 넘겨 `coderabbit review --agent`를 한 번 실행하고, 0 issues이면 `CodeRabbit Subagent Review: PASS`, 이슈가 있으면 `CodeRabbit Subagent Review: NEEDS_FIX` marker를 남깁니다.
