...
STUDYPOT_AI_OPENAI_MAX_OUTPUT_TOKENS_CURRICULUM_GENERATE=16384
STUDYPOT_AI_OPENAI_MAX_OUTPUT_TOKENS_RETROSPECTIVE_FEEDBACK=2048
STUDYPOT_AI_OPENAI_MAX_OUTPUT_TOKENS_TEAM_LEAD_CHAT=1536
STUDYPOT_AI_OPENAI_REASONING_EFFORT_DETAIL_KEYWORD_SUGGEST=minimal
```
...
STUDYPOT_AI_OPENAI_REASONING_EFFORT_DETAIL_KEYWORD_SUGGEST
```
...
- `STUDYPOT_AI_OPENAI_MAX_OUTPUT_TOKENS_CURRICULUM_GENERATE`: 커리큘럼 생성 출력 상한. 1년 초과 스터디도 전체 주차 JSON을 받을 수 있도록 기본값은 `16384`.
...
- `STUDYPOT_AI_OPENAI_REASONING_EFFORT_DETAIL_KEYWORD_SUGGEST`: GPT-5 chat-completions 세부 키워드 추천 reasoning effort. GMS nano 모델이 출력 토큰을 추론에만 쓰지 않도록 운영 기본값은 `minimal`이다.
