...
9. merge 전 `scripts/task/verify-pr-ready.sh <PR_NUMBER>`가 PR 상태와 최신-head Copilot review/thread 상태를 검증한다.
...
- 최신 head에 GitHub Copilot review activity가 없음
...
- 알림 전 `verify-copilot-review.sh`가 최신 head에 `copilot-pull-request-reviewer` activity와 미해결 Copilot review thread 0개를 확인한다.
