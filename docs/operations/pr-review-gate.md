...
11. `scripts/task/finish-pr.sh <PR_NUMBER>`는 최신 head와 review gate를 재검증하고 Mattermost incoming webhook으로 manual merge 알림을 보낸다.
12. 사용자가 GitHub에서 직접 merge 버튼을 누른다.
13. GitHub merge 후 Jira Task는 자동으로 완료 처리된다.
14. merge 이후 `scripts/task/finish-pr.sh cleanup-merged <PR_NUMBER>`가 develop sync, worktree cleanup, branch cleanup, Jira 상태 기록을 수행한다.
...
- 이 수동 merge 알림은 required `review-gate-pass` check와 최신 head CodeRabbit review marker가 모두 충족된 뒤에만 전송한다.
