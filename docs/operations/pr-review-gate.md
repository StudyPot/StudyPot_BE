...
7. GitHub Copilot review는 최신 PR head 기준으로 기본 최대 600초 동안 기다린다. 제출되면 actionable Copilot feedback을 코드, 테스트, 또는 문서로 반영하고 Copilot review thread를 모두 resolve한다.
...
- 최신 PR head에 대한 GitHub Copilot review activity가 없음
...
- 알림 전 `verify-copilot-review.sh`가 최신 PR head에 대한 GitHub Copilot review를 기본 최대 600초 동안 기다린 뒤 `copilot-pull-request-reviewer` activity와 미해결 Copilot review thread 0개를 확인한다. 기본 poll 간격은 30초이며 `STRICT_COPILOT_REVIEW_WAIT_SECONDS`, `STRICT_COPILOT_REVIEW_POLL_INTERVAL_SECONDS`로 조정할 수 있다.
- 최신 head Copilot review 요구는 기본값이다. bootstrap/harness 예외처럼 이전 head의 Copilot activity를 허용해야 하는 경우에만 `STRICT_REQUIRE_LATEST_HEAD_COPILOT_REVIEW=0`을 명시한다.
