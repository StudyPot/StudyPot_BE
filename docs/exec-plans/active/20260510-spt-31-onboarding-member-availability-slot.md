...
- CodeRabbit review `NEEDS_FIX` 5건 대응:
  - `member_availability_slot` 교체 삭제를 hard delete에서 `deleted_at` 기반 soft delete로 변경.
  - `AvailabilitySlotCommand.validate()`에 day/time/timezone 검증 추가.
  - controller request DTO에 `HH:mm` time pattern 검증과 timezone custom constraint 추가.
  - command/controller 검증 테스트 추가.
