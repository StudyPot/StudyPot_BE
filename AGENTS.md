...
- Once implementation starts, continue through code, tests, review feedback, PR creation, review gates, auto-merge, and post-merge cleanup until the feature is complete or a real blocker requires user input.
...
- After passing the PR review gate, use `scripts/task/finish-pr.sh` to verify readiness, auto-merge PRs, send the Korean Mattermost merge-complete notification when configured, and run safe local cleanup.
- `scripts/task/finish-pr.sh cleanup-merged <PR_NUMBER>` remains available for externally merged PRs or interrupted finish recovery; it performs develop sync, worktree cleanup, branch cleanup, and idempotent Jira state recording.
...
- Do not run the auto-merge finish path until the latest PR head has a `CodeRabbit Subagent Review: PASS` marker or a `CodeRabbit Subagent Review: ADDRESSED` marker with evidence.
...
- `scripts/task/finish-pr.sh` verifies that the PR head did not change during verification, auto-merges the PR, and then runs safe cleanup.
- GitHub Actions transitions the linked Jira Task to the done status after merge; `scripts/task/finish-pr.sh` also records Jira completion idempotently during cleanup.
